...
function PriceTheme5(props: Props) {
	...
	return (
		<div className="price-theme5-root text-[#585656]">
			...
			{(dealer_discount_label || dealer_additional_label || dealer_sale_price_label || incentive_discount_label) && (
				<div className="price-theme5-breakdown-container -ml-4" style={{ width: 'calc(100% + 32px)' }}>
					{/* Dealer Discounts */}
					{dealer_discount_label &&
						dealer_discount_details.map((el) => (
							<div key={el.title} className="price-theme5-discount-row flex justify-between px-4 py-2">
								<span className="price-theme5-discount-title text-sm font-medium text-black">{el.title}</span>
								<span className="price-theme5-discount-value text-sm font-medium text-black">{el.value}</span>
							</div>
						))}
					...
					{dealer_additional_label &&
						dealer_additional_details.map((el) => (
							<div key={el.title} className="price-theme5-additional-row flex justify-between px-4 py-2">
								<span className="price-theme5-additional-title text-sm font-medium text-black">{el.title}</span>
								<span className="price-theme5-additional-value text-sm font-medium text-black">{el.value}</span>
							</div>
						))}
					...
					{incentive_discount_label &&
						incentive_discount_details.map((el) => (
							<div key={el.title} className="price-theme5-incentive-row flex justify-between px-4 py-2">
								<span className="price-theme5-incentive-title text-sm font-medium text-black">{el.title}</span>
								<span className="price-theme5-incentive-value text-sm font-medium text-black">{el.value}</span>
							</div>
						))}
				</div>
			)}
			...
			{sale_price_label && !retail_price_label && (
				<div className="price-theme5-final-row flex justify-between px-4 py-2">
					<span className="price-theme5-final-label text-sm font-medium text-black">{sale_price_label}</span>
					<span className="price-theme5-final-value text-sm font-medium text-black">{sale_price_formatted}</span>
				</div>
			)}
			...
			{sale_price_label && retail_price_label && (
				<div className="price-theme5-final-highlight-row flex items-center justify-between py-2">
					<div className="price-theme5-final-highlight-label uppercase text-black text-h5">{sale_price_label}</div>
